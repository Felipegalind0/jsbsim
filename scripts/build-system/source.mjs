...
export async function contentManifest(root, { roots = ["."], exclude = () => false, ignoreBuildDirectories = false } = {}) {
  const files = [];
...
export async function nativeInputRoots(root) {
  ...
  for (const file of new Set(paths)) {
    try { await lstat(path.join(root, file)); retained.push(safeRelative(file)); }
...
function tarText(buffer) { return buffer.toString("utf8").split("\0", 1)[0]; }
function paxFields(buffer) {
  const fields = {};
  for (let offset = 0; offset < buffer.length;) {
    const space = buffer.indexOf(32, offset), length = Number(buffer.subarray(offset, space).toString());
    if (space < offset || !Number.isInteger(length) || length <= space - offset + 1 || offset + length > buffer.length) throw new Error("Invalid PAX record.");
    const text = buffer.subarray(space + 1, offset + length - 1).toString("utf8"), equals = text.indexOf("=");
    if (equals < 1) throw new Error("Invalid PAX field.");
    fields[text.slice(0, equals)] = text.slice(equals + 1);
    offset += length;
  }
  return fields;
}

/** Extract only regular files/directories; never execute tar hooks or follow links. */
export async function extractSourceArchive(archive, destination) {
  const packed = await readFile(archive);
  const bytes = packed[0] === 0x1f && packed[1] === 0x8b ? gunzipSync(packed) : packed;
  let localPax = {}, globalPax = {};
  const seen = new Set();
  await mkdir(destination, { recursive: true });
  if ((await readdir(destination)).length) throw new Error("Archive destination must be empty.");
  for (let offset = 0; offset + 512 <= bytes.length;) {
    const header = bytes.subarray(offset, offset + 512);
    if (header.every(value => value === 0)) break;
    const sizeText = tarText(header.subarray(124, 136)).trim();
    if (!/^[0-7]+$/.test(sizeText)) throw new Error("Unsupported archive entry size.");
    const size = Number.parseInt(sizeText, 8), type = String.fromCharCode(header[156]);
    const bodyStart = offset + 512, bodyEnd = bodyStart + size;
    if (!Number.isSafeInteger(size) || bodyEnd > bytes.length) throw new Error("Truncated source archive.");
    const body = bytes.subarray(bodyStart, bodyEnd);
    offset = bodyStart + Math.ceil(size / 512) * 512;
    if (type === "g") { globalPax = { ...globalPax, ...paxFields(body) }; continue; }
    if (type === "x") { localPax = paxFields(body); continue; }
    const name = tarText(header.subarray(0, 100)), prefix = tarText(header.subarray(345, 500));
    const relative = safeRelative(localPax.path ?? globalPax.path ?? (prefix ? prefix + "/" + name : name));
    localPax = {};
    if (!["0", "\0", "5"].includes(type)) throw new Error("Unsupported source archive entry: " + relative);
    const target = path.join(destination, relative);
    if (type === "5") { await mkdir(target, { recursive: true }); continue; }
    if (seen.has(relative)) throw new Error("Duplicate archive file: " + relative);
    seen.add(relative);
    await mkdir(path.dirname(target), { recursive: true });
    const mode = Number.parseInt(tarText(header.subarray(100, 108)).trim(), 8);
    await writeFile(target, body, { flag: "wx", mode: mode & 0o111 ? 0o755 : 0o644 });
  }
  if (!seen.size) throw new Error("Empty source archive.");
}

export function validateSourceLock(lock) {
  if (lock?.schemaVersion !== 1 || typeof lock.origin !== "string" || !/^https:\/\/github\.com\/[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+(?:\.git)?$/.test(lock.origin) ||
      !/^[a-f0-9]{40}$/.test(lock.commit ?? "") || /^0+$/.test(lock.commit) ||
      !HEX.test(lock.archive?.sha256 ?? "") || /^0+$/.test(lock.archive?.sha256 ?? "") ||
      !HEX.test(lock.contentSha256 ?? "") || /^0+$/.test(lock.contentSha256 ?? "")) throw new Error("Invalid or placeholder JSBSim source lock.");
  safeRelative(lock.archive.path);
  return lock;
}

export async function resolveNativeSource(sdkRoot, { mode, localSource, cacheRoot }) {
  const lock = validateSourceLock(JSON.parse(await readFile(path.join(sdkRoot, "jsbsim-source.lock.json"), "utf8")));
  if (mode === "local") {
    if (!localSource) throw new Error("Local mode requires --jsbsim-source.");
    const originRoot = path.resolve(sdkRoot, localSource);
    const capture = await captureSource(originRoot, path.join(cacheRoot, "native"), { rootsReader: nativeInputRoots });
    return { ...capture, origin: lock.origin, archiveSha256: null, archivePath: null, mode };
  }
  if (mode !== "pinned" || localSource) throw new Error("Conflicting or unsupported source mode.");
  const archive = path.resolve(sdkRoot, safeRelative(lock.archive.path));
  const packed = await readFile(archive);
  if (sha256(packed) !== lock.archive.sha256) throw new Error("Native archive checksum does not match source lock.");
  const destination = path.join(cacheRoot, "native", lock.contentSha256);
  await mkdir(path.dirname(destination), { recursive: true });
  const staging = await mkdtemp(path.join(path.dirname(destination), ".archive-"));
  try {
    await extractSourceArchive(archive, staging);
    const manifest = await contentManifest(staging);
    if (manifestSha256(manifest) !== lock.contentSha256) throw new Error("Extracted source content does not match source lock.");
    try { await rename(staging, destination); }
    catch (error) {
      if (!["EEXIST", "ENOTEMPTY"].includes(error.code)) throw error;
      await verifyContent(destination, manifest);
    }
    return { root: destination, origin: lock.origin, commit: lock.commit, dirty: false,
      contentSha256: lock.contentSha256, archiveSha256: lock.archive.sha256, archivePath: lock.archive.path, manifest, mode };
  } finally { await rm(staging, { recursive: true, force: true }); }
}

export const SDK_INPUT_ROOTS = ["src", "bindings", "scripts", "cmake", "test", "bench", "docs", "demo", "patches", ".github", ".gitignore",
  "package.json", "package-lock.json", "tsconfig.json", "tsup.config.ts", "LICENSE", "README.md", "jsbsim-source.lock.json", "build-toolchain.lock.json"];
export const sdkCaptureOptions = { roots: SDK_INPUT_ROOTS, ignoreBuildDirectories: true,
  exclude: relative => ["src/generated", "demo/public/wasm", "demo/public/sdk", "demo/vite.config.js", "demo/vite.config.d.ts"].includes(relative) || relative.startsWith("demo/public/.sdk-sync-") || relative.endsWith(".tsbuildinfo") };
...
export function buildInputSha256(identity) {
  ...
  return sha256(JSON.stringify({
    ...
    sdk: { commit: i.sdk.commit, contentSha256: i.sdk.contentSha256, dirty: i.sdk.dirty },
    mode: i.build.mode,
...
export async function readDescriptor(file) {
  if (!file) throw new Error("A resolved JSBSIM_BUILD_DESCRIPTOR is required; use npm run build or build:local.");
  const descriptor = JSON.parse(await readFile(file, "utf8")), identity = descriptor.identity;
  if (descriptor.schemaVersion !== 1 || identity?.schemaVersion !== 1 || !HEX.test(identity.build?.inputSha256 ?? "") ||
      ...
      !["pinned", "local"].includes(identity.build.mode)) throw new Error("Invalid build descriptor.");
  ...
  if (descriptor.native.mode !== identity.build.mode || (identity.build.mode === "pinned" && (identity.native.dirty || identity.sdk.dirty))) {
    throw new Error("Build source mode mismatch or dirty pinned input.");
  }
...
export function verifyCmakeCache(descriptor, cache) {
  if (cache.match(/^JSBSIM_SOURCE_DIR:[^=]*=(.+)$/m)?.[1] !== descriptor.native.root ||
...
export async function verifyWorkspaceInputs(descriptor) {
  const excluded = new Set(["src/build-identity.ts"]);
  const options = { ...sdkCaptureOptions,
    exclude: relative => sdkCaptureOptions.exclude(relative) || excluded.has(relative) };
  const actual = await contentManifest(descriptor.workspaceRoot, options);
  const expected = descriptor.sdk.manifest.filter(file => !excluded.has(file.path));
...
export function verifyResolvedSourceLock(native, frozenLock) {
  validateSourceLock(frozenLock);
  if (native.origin !== frozenLock.origin || (native.mode === "pinned" &&
      (native.commit !== frozenLock.commit || native.contentSha256 !== frozenLock.contentSha256 ||
       native.archiveSha256 !== frozenLock.archive.sha256 || native.archivePath !== frozenLock.archive.path))) {
    throw new Error("Resolved native source differs from the frozen SDK source lock.");
  }
}

...
try {
  ...
  for (const phase of ["write", "restore-and-delete", "verify-deletion"]) {
    ...
    const result = await page.evaluate(async phase => {
      const { GEAR_CONTACT_FIELDS, JSBSimSdk, buildIdentity } = await import("/dist/index.js");
      ...
      try {
        ...
        // The same artifact must still carry the downstream hand-written bindings.
        check(typeof sdk.module.PropertyBatch === "function", "Built runtime lost its PropertyBatch binding");
        check(typeof sdk.module.GearContacts === "function", "Built runtime lost its GearContacts binding");
        check(GEAR_CONTACT_FIELDS.length > 0, "SDK bundle lost its gear-contact field list");
        if (phase === "write") {
  ...
  console.log(JSON.stringify({ status: "passed", browser: browser.version(), identity,
    checks: ["IDBFS linked", "text and binary survive navigation", "deletions and updates persist",
      "downstream PropertyBatch/GearContacts bindings retained", "executive disposed after each phase"],

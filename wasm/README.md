...
The local fork package is `@felipegalind0/jsbsim`, `1.2.4-fork.7`. It is not published by this workflow. Install the exact checked tarball recorded in `build/last-package.json`; the native revision is identified separately by `buildIdentity.native.commit`.

...
export async function captureRepository(sdkRoot, { allowDirty = false, cacheRoot } = {}) {
  ...
  const native = { ...capture, mode: "in-tree", archiveSha256: null, archivePath: null };
...
export async function verifyWorkspaceInputs(descriptor) {
  const outputDirectories = ["node_modules", "dist", "build", "generated", "src/generated", "demo/node_modules", "demo/dist", "demo/public/sdk", "demo/public/wasm"];
  const generated = relative => relative === "src/build-identity.ts" ||
    outputDirectories.some(directory => relative === directory || relative.startsWith(directory + "/")) ||
    relative.startsWith("demo/public/.sdk-sync-") || relative === "demo/vite.config.js" ||
    relative === "demo/vite.config.d.ts" || relative.endsWith(".tsbuildinfo");
